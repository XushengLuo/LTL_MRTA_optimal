...
class buchi_graph(object):
    ...
    def __init__(self, formula, formula_comp, contra):
    ...
    def execLtl2ba(self):
        ...
        self.buchi_str = subprocess.check_output('/Users/chrislaw/Github/LTL_MRTA_optimal'
                                                 + "/./ltl2ba -f \"" + self.formula + "\"", shell=True).decode("utf-8")
        ...
        for state in state_group:
            ...
            if state_if_fi:
                ...
                for (label, state_dest) in relation_group:
                    ...
                    # add edge
                    label = self.contradiction(label)
                    ...
                    label = self.InitialDelInfesEdge(label)
                    if not label or label.isspace():
                        continue

                    self.buchi_graph.add_edge(state, state_dest, label=label, dnf=to_dnf(label))
            else:
                state_skip = re.findall(state + r':\n\tskip\n', self.buchi_str, re.DOTALL)
                if state_skip:
                    self.buchi_graph.add_edge(state, state, label='(1)', dnf=to_dnf('1'))
    ...
    def contradiction(self, orig_label):
        if orig_label == '(1)':
            return orig_label

        symbol = orig_label.replace('||', '|').replace('&&', '&').replace('!', '~')
        symbol = to_dnf(symbol).__str__()
        clause = symbol.split(' | ')
        # merge
        feas = []
        for index, c in enumerate(clause):
            literals_ = c.strip('(').strip(')').split(' & ')
            literals_ = [l for l in literals_ if '~' not in l]
            contra = False
            for pair in self.pair:
                if pair[0] in literals_ and pair[1] in literals_:
                    contra = True
                    break
            if not contra:
                feas.append(index)

        orig_label = '|'.join([clause[index] for index, c in enumerate(clause) if index in feas])
    ...
    def InitialDelInfesEdge(self, orig_label):
        ...
        for index, c in enumerate(clause):
            ...
            literals_ = [l for l in literals_ if '~' not in l]
            robot = []
            for i in range(len(literals_)):
                robot.append(literals_[i].split('_')[1])
            if len(robot) == len(set(robot)):
                feas.append(index)
    ...
    def DelInfesEdge(self):
        ...
        for edge in self.buchi_graph.edges():
            ...
            if b_label != '(1)':
                ...
                truth_table = dict()
                for literal in exp.split('|')[0].split(' & '):
                    l = literal.strip('(').strip(')')
                    if '~' in literal:
                        truth_table[l[1:]] = False
                    else:
                        truth_table[l] = True
        ...
        for node1 in self.buchi_graph.nodes():
            for node2 in self.buchi_graph.nodes():
                if node1 != node2 and 'accept' in node2:
                    try:
                        l, p = nx.algorithms.single_source_dijkstra(self.buchi_graph, source=node1, target=node2)
                    ...
                    for succ in self.buchi_graph.succ[node1]:
                        try:
                            l0, p = nx.algorithms.single_source_dijkstra(self.buchi_graph, source=succ, target=node1)
                        except nx.exception.NetworkXNoPath:
                            l0 = np.inf
                            # path0 = []
                        if l0 < l and p:
