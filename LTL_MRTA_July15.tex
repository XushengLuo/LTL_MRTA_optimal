...
\begin{align}\label{eq:same_suffix}
  ...
  \end{align}
\endgroup
where $v_k^b$ is the $b$-th vertex in the set of vertices in the routing graph $\ccalG$  that are associated with literal $\ap{i}{j}{k,\chi}$. If the clause $\ccalC_p^\gamma$ is true, i.e.,  $b_p=1$, the $b$-th vertex $v_k^b$ is visited by the $b$-th robot $r^b \in \ccalK^\chi$. On the other hand, if $\ccalK^\chi = \emptyset$, that is, literals that share this $\chi$ do not appear in the prefix part, we use constraint~\eqref{eq:same} to impose the same-$\ag{i}{j}$ constraints. Note that by condition (d) in Definition~\ref{defn:run}, all literals with non-zero connectors appearing in the edge labels, e.g., $\gamma(\vertex{prior}, \vertex{accept})$ in Fig.~\ref{fig:lasso}, of subtasks in $P_{\text{min}}$, also appear in the single clause of the edge label, e.g., $\gamma(\vertex{prior}', \vertex{accept})$ in Fig.~\ref{fig:lasso}, that the final locations of the prefix part satisfies. Therefore, when robots head back towards their initial locations, the same-$\ag{i}{j}$ constraints over the last-completed subtask is satisfied automatically. In other words, robots can safely return to their initial locations at last without violating the same-$\ag{i}{j}$ constraints.
...
After solving the MILP for the suffix part, we can use the method in Section~\ref{sec:solution2mrta} to obtain low-level paths which drives the transition back to $v_{\text{accept}}$ and return to initial locations to close the trajectories.
