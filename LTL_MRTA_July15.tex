...
Given 4 different vertices $v_1, v_2, v_3, v_4$ in the NBA such that $v_1 \xrightarrow{\gamma}_B v_2 \xrightarrow{\gamma'}_B v_3$, we say that these 4 vertices have the ID property if {\it (i)} $\gamma(v_1) = \gamma(v_2) = \gamma(v_4)$,  {\it (ii)} $v_1 \xrightarrow{\gamma'}_B v_4 \xrightarrow{\gamma}_B v_3$,  {\it (iii)} $v_1 \xrightarrow{\gamma \wedge \gamma'}_B v_3$, {\it (iv)} $\gamma$ and $\gamma'$ are compatible with $\gamma(v_2)$ and {\it (v)} $v_3 \neq \vertex{accept}$.
\end{defn}
Intuitively, if vertices have the ID property (see Fig.~\ref{fig:id}), we have equivalent subtasks $(v_1, v_2) \sim (v_4, v_3)$ and $(v_1, v_4) \sim (v_2, v_3)$. We refer to $(v_1, v_3)$ as the {\it composite} subtask and $(v_1, v_2)$, $(v_1, v_4)$ as the {\it elementary} subtasks.  Although both can lead to vertex $v_3$, elementary subtasks are preferred since subtasks $(v_1, v_2)$ and $(v_1, v_4)$ can be executed independently, making it easier to satisfy and more flexible to implement, when compared to the composite subtask that requires that two subtasks are accomplished at the same time. The last two conditions {\it (iv)} and {\it (v)}, echoing with conditions (c)  and (d) in the Definition~\ref{defn:run},  ensures that the removing of composite subtasks will not empty the restricted set of accepting runs, as shown in Proposition~\ref{prop:sub-NBA} in Section~\ref{sec:correctness}.
\begin{defn}[(Sequential triangle property)]\label{defn:st}
  Given 3 different vertices $v_1, v_2, v_3$ in the NBA such that $v_1 \xrightarrow{\gamma}_B v_2 \xrightarrow{\gamma'}_B v_3$, we say that these 3 vertices $v_1, v_2, v_3$ have the ST property if {\it (i)} $\gamma(v_1) = \gamma(v_2)$, {\it (ii)} $v_1 \xrightarrow{\gamma \wedge \gamma'}_B v_3$. {\it (iii)}  $\gamma$ is compatible with $\gamma(v_2)$ and {\it (iv)} $v_3 \neq \vertex{accept}$.
\end{defn}
...
\end{figure}
\begin{rem}
  The last two conditions {\it (iv)} and {\it (v)} in the definition of ID property and the last two conditions {\it (iii)} and {\it (iv)} in the definition of ST property are necessary for establishing the non-emptiess of restricted set of accepting runs after the prunning of composite subtasks. However, these conditions are not necessary in implementating the prunning process. We found that in the simulations in Section~\ref{sec:sim}, these conditions can be omitted without compromising  the preformance when checking the ID and ST properties.
\end{rem}
...
If $\ccalL^{\phi,v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{subtask}) \not= \emptyset$, then there exists a path $\tau$ that generates a word $w$ that incurs a run $\rho$ in $\auto{subtask}$ that connects $v_0$ and $\vertex{accept}$. If the run $\rho$ traverses a composite edge, we can locate the path segment $\tau_\text{seg}$ in $\tau$ that enables this subtask and replace $\tau_\text{seg}$ with path segment $\tau'_{\text{seg}}$ constructed as above, then the new path $\tau'$ will generate a word $w'$ that incurs a run $\rho'$ that traverses the two elementary edges that the composite edge corresponds to. Next, we prove that if run $\rho$ is in the restricted set, so is the run $\rho'$. Note that these two runs differ in that $\rho$



On the other hand, it the run does not traverse any composite edge, the generated word $w$ belongs to $\ccalL^{\phi,v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{subtask}^-)$.


which leads to the conclusion that if $\ccalL^{\phi,v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{subtask}) \not= \emptyset$, then $\ccalL^{\phi,v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{subtask}^-) \not= \emptyset$.
