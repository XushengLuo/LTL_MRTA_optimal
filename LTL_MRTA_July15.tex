...
\begin{defn}[(Restricted set of accepting runs)]\label{defn:run}
  Given a NBA $\autop$ that corresponds to a \ltl formula, the  restricted set of accepting runs includes any accepting run, in the prefix-suffix structure of $v_0, \ldots, \vertex{prior}, \vertex{accept} (\vertex{next}, \ldots, \vertex{prior}', \vertex{accept})^\omega$ (see Fig.~\ref{fig:lasso}), that meets the following conditions: (a)\label{cond:a} There only exists one initial vertex and one  accepting vertex in the accepting run, but different accepting runs can have different pairs of initial and accepting vertices; (b)\label{cond:b} In the prefix part of the accepting run, only initial and accepting vertice can be allowed to not have self-loops, that is, their vertex labels  can be $\bot$, and in the suffix part,  only the accepting vertex can be allowed to not have self-loops; %% (c)\label{cond:c} For any two consecutive vertices $v_1$, $v_2$ in the accepting run, if $v_2$ is not the accepting vertex $\vertex{accept}$ and it  has a self-loop, then the edge label $\gamma(v_1, v_2)$ strongly implies the end vertex label $\gamma(v_2)$, i.e., $ \gamma(v_1, v_2) \simplies_s \gamma(v_2)$;
...
where $d_{uv} \in \mathbb{N}$ is the distance between cells $u$ and $v$.
  ...
  Given a discrete environment $E$, the initial robot configurations and the specification $\phi$ of which the restricted set of accepting runs is non-empty, some words in the accepted language  $\ccalL(B)$ may not be possible to realize. Let $\ccalL_E(B)$ be the set that collect words in $\ccalL(\autop)$ that can be realized. Moreover, given a pair of initial and accepting vertices, $v_0$ and $v_\text{accept}$, let $\ccalL^{v_0 \scriptveryshortarrow \vertex{accept}}_E(B)$ collect finite words that can be realized and can generate a run in $B$ from $v_0$ to $\vertex{accept}$. The following propositions show that feasibility of Problem~\ref{prob:1} is preserved under the proposed LTL-MRTA algorithm. The proofs are shown in the Appendix.
  \begin{prop}\label{prop:prune}
    Given a discrete environment $E$, the initial robot configurations and the specification $\phi$, if $\ccalL_E(\autop) \not= \emptyset$, then $\ccalL_E(\auto{relax}) \not= \emptyset$.
  \end{prop}
  \begin{prop}\label{prop:inclusion}
 Given a discrete environment $E$, the initial robot configurations and the specification $\phi$, $\ccalL_E(\autop^-) \subseteq \ccalL_E(\auto{relax})$.
  \end{prop}
 \begin{prop}\label{prop:sub-NBA}
    Given a discrete environment $E$, the initial robot configurations and the specification $\phi$, for a pair of initial and accepting vertices, $v_0$ and $\vertex{accept}$, suppose an obstacle-free path exists between any two regions. If $\ccalL^{v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{relax}) \not= \emptyset$, then $\ccalL^{v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{subtask}^-) \not= \emptyset$.
 \end{prop}
