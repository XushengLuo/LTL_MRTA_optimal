...
\paragraph{1) Mutual exclusion in $\mathsf{lits}^+(\cp{p}{\gamma})$:}\label{prune:exclusion1} We delete the clause $\cp{p}{\gamma} \in \mathsf{cls}(\gamma)$ (replacing it with $\bot$) if there exist two positive literals $\ap{i}{j}{k,\chi}, \ap{i}{j}{k',\chi} \in \mathsf{lits}^+(\cp{p}{\gamma})$ such that $k\not= k'$ and $\chi\not=0$, since the same group of robots cannot be at different regions at the same time.
 ...
 \paragraph{4) Absorption in $\mathsf{lits}^-(\cp{p}{\gamma})$:} \label{prune:absorption2} Similarly, we delete the negative literal $\neg \ap{i}{j}{k} \in \mathsf{lits}^-(\cp{p}{\gamma})$ (replacing it with $\top$), if another $\neg \ap{i'}{j}{k} \in \mathsf{lits}^-(\cp{p}{\gamma})$ exists such that $i' < i$.
       ...
       \paragraph{7) Infeasible transitions:} \label{prune:infeasible} We delete all edges associated with $\bot$ label, that is, they cannot be enabled.  We also delete all vertices without self-loops except the initial and accepting vertices, which is justified by condition (b) in Definition~\ref{defn:run}. Finally, we delete every vertex, except the initial vertex, that cannot be reached by other vertices.
%\end{noindlist}
...
\begin{defn}[(Independent diamond property)]\label{defn:id}
  Given 4 different vertices $v_1, v_2, v_3, v_4$ in the NBA $\auto{relax}$ such that $v_1 \xrightarrow{\gamma}_B v_2 \xrightarrow{\gamma'}_B v_3$, we say that these 4 vertices have the ID property if (a) $\gamma(v_1) = \gamma(v_2) = \gamma(v_4)$, (b) $v_1 \xrightarrow{\gamma'}_B v_4 \xrightarrow{\gamma}_B v_3$,  (c) $v_1 \xrightarrow{\gamma \wedge \gamma'}_B v_3$, (d) $\gamma_\phi$ and $\gamma_\phi'$ are compatible with both $\gamma_\phi(v_2)$ and $\gamma_\phi(v_3)$, and (e) $\gamma(v_3) = \top$ if $v_3 = \vertex{accept}$.
\end{defn}
Intuitively, if vertices have the ID property (see Fig.~\ref{fig:id}), we have equivalent subtasks $(v_1, v_2) \sim (v_4, v_3)$ and $(v_1, v_4) \sim (v_2, v_3)$ in $\auto{subtask}$. We refer to $(v_1, v_3)$ as the {\it composite} subtask and $(v_1, v_2)$, $(v_1, v_4)$ as the {\it elementary} subtasks.  Although both can lead to vertex $v_3$, elementary subtasks are preferred since subtasks $(v_1, v_2)$ and $(v_1, v_4)$ can be executed independently, making it easier to satisfy and more flexible to implement, when compared to the composite subtask that requires that two subtasks are accomplished at the same time. It's worth noting that the compatibility in (d) is conducted in terms of their corresponding labels in $\autop^-$. The last two conditions (d) and (e), echoing with conditions (c)  and (d) in the Definition~\ref{defn:run},  ensures that the removing of composite subtasks will not empty the restricted set of accepting runs, as shown in Proposition~\ref{prop:sub-NBA} in Section~\ref{sec:correctness}.
\begin{defn}[(Sequential triangle property)]\label{defn:st}
  Given 3 different vertices $v_1, v_2, v_3$ in the NBA $\auto{relax}$ such that $v_1 \xrightarrow{\gamma}_B v_2 \xrightarrow{\gamma'}_B v_3$, we say that these 3 vertices $v_1, v_2, v_3$ have the ST property if %% {\it (i)} $\gamma(v_1) = \gamma(v_2)$,
  (a) $v_1 \xrightarrow{\gamma \wedge \gamma'}_B v_3$, (b)  $\gamma_\phi$ is compatible with $\gamma_\phi(v_2)$ and  $\gamma'_\phi$ is compatible with $\gamma_\phi(v_3)$,  and (c) $\gamma(v_3) = \top$ if $v_3 = \vertex{accept}$.
\end{defn}

If vertices have the ST  property (see Fig.~\ref{fig:st}), subtask $(v_1, v_2)$ should be satisfied no later than $(v_2, v_3)$. Observe that if vertices $v_1, v_2, v_3, v_4$ have the ID property, then $v_1, v_2, v_3$ and $v_1, v_4, v_3$ have the ST property. Based on these two properties, for any edge $(v_1, v_3)$ in $\auto{subtask}$, we delete it if there exists  vertex $v_2$ such that combined they have  ST property. We denote by $\auto{subtask}^-$ the pruned $\auto{subtask}$.
...
\begin{rem}
  The last two conditions (d) and (e) in the definition of ID property and the last two conditions (b) and (c) in the definition of ST property are necessary for establishing the theorectical guarantee in Proposition~\ref{prop:sub-NBA}  on the non-emptiess of restricted set of accepting runs after the prunning of composite subtasks. However, these conditions are not necessary in implementating the prunning process. We found that in the simulations in Section~\ref{sec:sim}, with these conditions being omitted, only checking condition (a) of Definition~\ref{defn:st} in $\auto{subtask}$  does not compromise  the preformance..
\end{rem}
...
where $d_{uv} \in \mathbb{N}$ is the distance between cells $u$ and $v$.
 ...
 Given a discrete environment $E$, the initial robot configurations and the specification $\phi$, after the relaxation stage of replacing all negative literals with $\top$ in Section~\ref{sec:prune}, it holds that $\ccalL^\phi_E(\autop^-) \subseteq \ccalL^\phi_E(\auto{relax})$.
  ...
  \begin{prop}\label{prop:nonempty}
    Given a discrete environment $E$, the initial robot configurations and the specification $\phi$, for a pair of initial and accepting vertices, $v_0$ and $\vertex{accept}$, after the extraction of the sub-NBA $\auto{subtask}$ in Section~\ref{sub-NBA:1}, if $\ccalL^{\phi,v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{relax}) \not= \emptyset$, then $\ccalL^{\phi,v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{subtask}) \not= \emptyset$.
  \end{prop}
 ...
 \begin{prop}\label{prop:sub-NBA}
    Given a discrete environment $E$, the initial robot configurations and the specification $\phi$, for a pair of initial and accepting vertices, $v_0$ and $\vertex{accept}$, after the pruning steps in Section~\ref{sub-NBA:2}, if $\ccalL^{\phi,v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{subtask}) \not= \emptyset$, then $\ccalL^{\phi,v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{subtask}^-) \not= \emptyset$.
 \end{prop}
...
As for the last step~\hyperref[prune:infeasible]{\it 7)}, removing infeasible transitions and unreachable vertices will not exclude any realizable words. Furthermore, according to condition (b) in Definiction~\ref{defn:run}, any accepting run within the restricted set of $\autop$ does not contain vertices without self-loops except the initial and accepting vertices, thus, removing such vertices in step~\hyperref[prune:infeasible]{\it 7)} will not affect runs in the restricted set of $\autop$, which implies that $\ccalL^\phi_E(\autop^-) = \ccalL_E(\autop)$, completing the proof.
...
Given a pair of initial and accepting vertices, $v_0$ and $\vertex{accept}$, the correspondsing sub-NBA $\auto{subtask}$ is composed of all vertices and edges in $\auto{relax}$ that are in some paths that connect $v_0$ and $\vertex{accept}$, except that the self-loop of
$v_0$ (if exists) is removed if the initial robot locations do not satisfy its corresponding vertex label in $\autop^-$ along with the outgoing edges of the initial vertex if the initial robot locations do not satisfy their corresponding edge labels in $\autop^-$ (see Section~\ref{sec:sort}). Removing such self-loops and outgoing edges will not affect $\ccalL^{\phi,v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{relax})$ since any word $w$ that, at the beginning, satisfies the label of the initial vertex or labels of outgoing edges that are removed can not be generated by feasible paths since initial robot locations violate these labels. Therfore, if $\ccalL^{\phi,v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{relax}) \neq \emptyset$, then $\ccalL^{\phi,v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{subtask}) \neq \emptyset$, completing the proof.

\subsection{Proof of Proposition~\ref{prop:sub-NBA}}\label{app:sub-NBA}
Compared to $\auto{subtask}$, $\auto{subtask}^-$ is obtained by removing edges  that is decomposable according to independent diamond property and sequential triangle property. We focus on the 3 different vertices $v_1, v_2, v_3$ that have the ST property, and the same analysis can be applied to vertices that have the ID property. The goal is to prove that, given a path segment $\tau_{\text{seg}}$ that completes the subtask $(v_1, v_3)$, that is, driving the transition to vertex $v_3$ while satisfying the vertex label $\gamma(v_1)$ en route, we can construct another path segment $\tau_{\text{seg}}'$ by simply repeating the last waypoint of $\tau_{\text{seg}}$ for each robot such that the path segment $\tau_{\text{seg}}'$ can incur a run that traverse along edges $(v_1, v_2)$ and $(v_2, v_3)$. Specifically, according to the Definition~\ref{defn:st} regarding ST property, since the path segment $\tau_{\text{seg}}$ satisfies the edge label $\gamma(v_1, v_3)$ at the final time  and the vertex label $\gamma(v_1)$ en route, and $\gamma(v_1, v_3) = \gamma(v_1, v_2) \wedge \gamma(v_2, v_3)$, we have that the path segment $\tau_{\text{seg}}$ satisfies the edge label $\gamma(v_1, v_2)$ and thus, $\tau_{\text{seg}}$ can drives the transition to vertex $v_2$. At the next time instant, if robots remain idle, i.e., remaining where they are at the end of $\tau_{\text{seg}}$, the edge label $\gamma(v_2, v_3)$ can be satisfied as well since the last waypoint satisfies $\gamma(v_1, v_3)$. Therefore, the path segment $\tau_{\text{seg}}'$ can be constructed by concatenating $\tau_{\text{seg}}$ and its last waypoint.
...
These two runs differ only in that $\rho$ traverse $v_1$ and $v_3$ while $\rho'$ traverses $v_1$, $v_2$ and $v_3$. According to condition (b) in Definition~\ref{defn:st}, $\gamma_\phi (v_1, v_2)$  is compatible with $\gamma_\phi (v_2)$ and $\gamma_\phi(v_2, v_3)$ is compatible with $\gamma_\phi(v_3)$, thus , these two edges that $\rho'$ traverses satisfy the condition (c) in the Definition~\ref{defn:run} regarding the restricted set. Furthermore, if $v_3$ is not $\vertex{accept}$, then $\rho'_\phi$ can be the prefix part of the same accepting run to which $\rho_\phi$ is the prefix part by concatenating the path $\tau'$ with the path behind $\tau$ that incurs this accepting run. Therefore, in this case, $\ccalL^{\phi,v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{subtask}^-) \neq \emptyset$. Otherwise, if $v_3= \vertex{accept}$, according to condition (c)  in Definition~\ref{defn:st}, $\gamma_\phi(v_3) = \top$, thus, the path $\tau$ itself incurs a run $\rho$ of which the corresponding $\rho_\phi$ itself is an accepting run in the restrcited set. Thus, we also have $\ccalL^{\phi,v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{subtask}^-) \neq \emptyset$. Finally, if $v_3 = \vertex{accept}$ and $\gamma_\phi(v_3)\neq \top$, we do not remove the subtask $(v_1, v_3)$. Therfore, $\ccalL^{\phi,v_0\scriptveryshortarrow \vertex{accept}}_E(\auto{subtask})$ is not affected, which completes the proof.

\subsection{Proof of the Proposition}
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{routing_graph.pdf}
  \caption{Routing graph $\ccalG$. Vertices that are associated with initial robot locations are inside the orange circle. Each gray ellipse surrounds vertices that are associated with the same subtask, most of which consist of two smaller ellipses, one including vertices associated with the starting vertex label (blue ellipse), the other including vertices associated the edge label (green ellipse). The missing smaller ellipse implies the corresponding label is $\top$ or $\bot$. Each smaller ellipse are divided into several parts (solid line), each part including vertices that associated with the same clause. Furthermore, each part is divided into smaller parts (dashed line), each part corresponding vertices that are associated with the same literal. Edges  indicate that there are connected vertices between  ellipses. Orange edges represent that starting vertices are from those associated with initial robot locations, blud edges for prior subtasks and green edges for vertex labels of the same subtasks. Note that no edges exist inside any ellipse, since vertices inside one ellipse will be visited at the same time.}
  \label{fig:routing}
\end{figure}
